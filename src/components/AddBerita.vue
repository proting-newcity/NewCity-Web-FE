...
const publish = async () => {
  // Validate fields
  if (!title.value.trim()) {
    return Swal.fire({ icon: 'warning', title: 'Judul kosong', text: 'Silakan masukkan judul.', confirmButtonColor: '#588157' });
  }
  if (!content.value.trim()) {
    return Swal.fire({ icon: 'warning', title: 'Isi berita kosong', text: 'Silakan masukkan isi berita.', confirmButtonColor: '#588157' });
  }
  if (!photo.value) {
    return Swal.fire({ icon: 'warning', title: 'Foto belum diunggah', text: 'Silakan unggah foto.', confirmButtonColor: '#588157' });
  }
  if (!selectedCategory.value) {
    return Swal.fire({ icon: 'warning', title: 'Kategori belum dipilih', text: 'Silakan pilih satu kategori.', confirmButtonColor: '#588157' });
  }
  ...
  formData.append('id_kategori', selectedCategory.value);
  formData.append('foto', photo.value);

  try {
    await dataStore.postBerita(formData);
